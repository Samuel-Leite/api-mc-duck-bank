...
const User = {
  ...
  async updateUser1(id, userData) {
    ...
    try {
      const result = await client.query(
        "UPDATE users SET username = COALESCE($1, username), cpf = COALESCE($2, cpf), email = COALESCE($3, email), address = COALESCE($4, address), password = COALESCE($5, password) WHERE id = $6 RETURNING *",
  ...
  async deleteAllUsers() {
    ...
    try {
      const result = await client.query("DELETE FROM users RETURNING *");
